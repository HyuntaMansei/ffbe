...
class Automator:
    ...
    def play_multi(self, rep_time, num_of_players):
        ...
        for cnt in range(rep_time):
            self.log(f"Before battle stage. completed: {cnt} times")
            before_battle_cnt = 0
            self.debug("Trial number of sortie: ")
            sortie_trial_cnt = 1
            ...
            while (not self.my_locator.locate('next')) and self.running:
                ...
                self.my_locator.locate_and_click('yes')
                time.sleep(1)
            # after battle stage
